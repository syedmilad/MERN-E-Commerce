...
export const getLatestProduct = async (req, res, next) => {
    try {
        ...
        console.log("Cache has key 'latest-product':", myCache.has("latest-product"));
...
export const getSingleProduct = async (req, res, next) => {
    try {
        ...
        if (myCache.has(`product-${id}`)) {
            product = JSON.parse(myCache.get("product"));
        }
        else {
...
export const getAllProducts = async (req, res, next) => {
    try {
        ...
        const totalPages = Math.ceil(filterOnlyProducts.length / limit); /** It's give totalPages */
        return res.status(201).json({
            success: true,
            products,
            totalPages,
