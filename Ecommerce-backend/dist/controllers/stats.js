...
import { calculatePercentage, getInventories, getRecordsByDateRange, } from "../utils/features.js";
...
export const getDashboardStats = async (req, res, next) => {
    try {
        ...
        if (myCache.has("admin-stats")) {
            ...
            const lastSixMonthAgoOrderPromise = getRecordsByDateRange(Order, sixMonthAgo, today);
            const latestTransactionPromise = Order.find({})
                .select(["orderItems", "discount", "total", "status"])
                .limit(4);
            const [thisMonthProduct, lastMonthProdcuct, thisMonthUserD, lastMonthUserd, thisMonthOrdersd, lastMonthOrdersd, productsCount, usersCount, allOrders, lastSixMonthOrder, categories, femaleCounts, latestTransaction,] = await Promise.all([
                thisMonthProducts,
                lastMonthProducts,
                thisMonthUsers,
                lastMonthUsers,
            ...
            const thisMonthRevenue = thisMonthOrdersd.reduce((total, order) => total + (order.total || 0), 0);
            const lastMonthRevenue = lastMonthOrdersd.reduce((total, order) => total + (order.total || 0), 0);
            const revenueInNumber = allOrders.reduce((total, order) => total + (order.total || 0), 0);
            ...
            stats = {
                changePercent: {
                    ...
                    product: calculatePercentage(thisMonthProduct.length, lastMonthProdcuct.length),
                    user: calculatePercentage(thisMonthUserD.length, lastMonthUserd.length),
                    order: calculatePercentage(thisMonthOrdersd.length, lastMonthOrdersd.length),
                },
                count: {
...
export const getPieCharts = async (req, res, next) => {
    try {
        ...
        if (myCache.has("admin-stats-pie")) {
            charts = JSON.parse(myCache.get("admin-stats-pie"));
        }
        else {
            const [processingOrder, shippedOrder, deliveredOrder, categories, productsCount, outOfStock, allOrders, allUser, adminCounts, userCounts,] = await Promise.all([
